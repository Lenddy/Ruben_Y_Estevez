		...
		<div>
			...
			<form className="from-group" onSubmit={submitHandler}>
				...
				<div>
					...
					) : formInfoErr.clientName ? (
						<p style={{ color: "red" }}>
							{" "}
							{formInfoErr.clientName.message}{" "}
						</p>
					) : null}
				</div>
				...
				<div>
					...
					<input
						...
						onChange={changeHandler}
					/>
					{info.dateAdded?.length > 0 &&
					info.dateAdded?.length < 2 ? (
						<p style={{ color: "red" }}>
							debes de agregar el dia que el préstamo fue echo
						</p>
					) : formInfoErr.dateAdded ? (
						<p style={{ color: "red" }}>
							{" "}
							{formInfoErr.dateAdded.message}{" "}
						</p>
					) : null}
				</div>
				...
				<div>
					...
					{formInfoErr.loanAmount ? (
						<p style={{ color: "red" }}>
							{" "}
							{formInfoErr.loanAmount.message}{" "}
						</p>
					) : null}
				</div>
				...
				<div>
					...
					<input
						type="number"
						name="interest"
						defaultValue={parseInt(10)}
						placeholder={parseInt(10)}
						className="form-control"
						step="any"
						onChange={changeHandler}
					/>
					{info.interest?.length > 0 && info?.interest < 0 ? (
						<p style={{ color: "red" }}>
							debes de poner la tasa de interés
						</p>
					) : formInfoErr.interest ? (
						<p style={{ color: "red" }}>
							{" "}
							{formInfoErr.interest.message}{" "}
						</p>
					) : null}
				</div>
				...
				<div>
					...
					<input
						...
						onChange={changeHandler}
					/>
					{info.latenessInterest?.length > 0 &&
					info?.latenessInterest < 0 ? (
						<p style={{ color: "red" }}>
							debes de poner la tasa de interés por mora
						</p>
					) : formInfoErr.interest ? (
						<p style={{ color: "red" }}>
							{" "}
				...
				<div>
					...
					<input
						...
						onChange={changeHandler}
					/>
					{formInfoErr.cuotasNumber ? (
						<p style={{ color: "red" }}>
							{" "}
							{formInfoErr.cuotasNumber.message}{" "}
						</p>
					) : null}
				</div>
				...
				{info?.client_id == null || info?.client_id == null ? (
					""
				) : (
					<ConfirmLoan
						formInfo={info}
						id={info.client_id}
						date={info.dateAdded}
						Amount={info.loanAmount}
						int={info.interest}
						cuotas={info.cuotasNumber}
						type={info.timeType}
					/>
				)}
